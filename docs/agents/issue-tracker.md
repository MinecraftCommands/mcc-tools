...
- **Read an issue**: `gh issue view <number> --comments` for a human-readable dump. To parse it, use `--json` instead — `--comments` emits markdown and cannot be combined with `--jq` (`gh` errors with ``cannot use `--jq` without specifying `--json` ``):
